...
const Navbar = ({ view, setView, user, handleLogout }) => {
  ...
  const NavItem = ({ target, label, icon: Icon }) => (
    <button 
      ...
      className={`flex items-center gap-2 px-4 py-2 rounded-xl transition font-medium ${
        view === target ? 'bg-purple-100 text-purple-800' : 'text-slate-600 hover:text-purple-600 hover:bg-purple-50'
      }`}
    >
      {Icon && <Icon size={18} />}
      {label}
  ...
  return (
    <nav className="bg-white/90 backdrop-blur-md border-b border-purple-100 sticky top-0 z-40 shadow-sm">
