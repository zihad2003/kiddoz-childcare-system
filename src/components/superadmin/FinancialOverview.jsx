...
const FinancialOverview = () => {
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center py-32 bg-white rounded-3xl border-2 border-dashed border-slate-100 animate-pulse">
            <div className="p-4 bg-primary-50 rounded-full mb-4">
                <DollarSign className="w-8 h-8 text-primary-600 animate-bounce" />
            </div>
    ...
    const COLORS = ['#6968A6', '#8280b3', '#9a97c0', '#bcbad5', '#dddce9'];
    ...
    return (
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-1000">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
                <div>
                    <div className="flex items-center gap-3 mb-3">
                        <div className="bg-primary-600 text-white p-2 rounded-lg shadow-lg shadow-primary-200">
                            <Target size={20} />
                        </div>
                        <Badge color="bg-primary-100 text-primary-700 font-black tracking-widest text-[9px]">FISCAL YEAR 2026</Badge>
                    </div>
                ...
                <div className="flex gap-4 w-full lg:w-auto">
                    ...
                    <Button variant="primary" className="flex-1 lg:flex-none h-14 font-black uppercase tracking-widest text-[10px] bg-slate-900 border-none shadow-2xl shadow-slate-200 rounded-2xl flex items-center gap-3">
                        <FileText size={18} className="text-primary-400" /> Fiscal Audit
                    </Button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {[
                    { label: 'Platform Revenue', val: `৳${financialData.summary.totalRevenue.toLocaleString()}`, trend: '+14.2%', up: true, icon: DollarSign, color: 'from-primary-600 to-primary-700', text: 'text-white' },
                    ...
                    <Card key={i} className={`p-8 border-none shadow-2xl rounded-[2.5rem] relative overflow-hidden group hover:-translate-y-2 transition-all duration-500 ${stat.color === 'bg-white' ? 'bg-white' : `bg-gradient-to-br ${stat.color}`}`}>
                        ...
                        <div className="flex justify-between items-start mb-6">
                            ...
                            <div className={`p-3 rounded-2xl ${stat.text === 'text-white' ? 'bg-white/10' : 'bg-slate-50'} transition-transform group-hover:scale-110`}>
                                <stat.icon size={18} className={stat.text === 'text-white' ? 'text-white' : 'text-primary-600'} />
                            </div>
                        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                    ...
                    <div className="h-96 w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={financialData.monthlyRevenue}>
                                ...
                                <Tooltip
                                    cursor={{ stroke: '#6968A6', strokeWidth: 2, strokeDasharray: '3 3' }}
                                    contentStyle={{ borderRadius: '1.5rem', border: 'none', boxShadow: '0 30px 60px -12px rgba(0,0,0,0.15)', padding: '20px', fontWeight: '900' }}
                                />
                                <Area type="monotone" dataKey="revenue" stroke="#6968A6" strokeWidth={5} fillOpacity={1} fill="url(#colorRev)" />
                ...
                <div className="space-y-8">
                    <Card className="p-8 border-none shadow-2xl bg-white rounded-[3rem]">
                        ...
                        <div className="h-48 w-full relative mb-8">
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                                <Activity size={24} className="text-primary-100" />
                            </div>
                        </div>

                        <div className="space-y-3">
                            {revenueData.map((item, index) => (
                                <div key={item.name} className="flex items-center justify-between p-3.5 bg-slate-50 rounded-2xl group hover:bg-primary-50 transition-colors cursor-default">
                                    <div className="flex items-center gap-3">
                                        <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: COLORS[index % COLORS.length] }}></div>
                                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest group-hover:text-primary-700 transition-colors">{item.name}</span>
                                    </div>
                    ...
                    <Card className="p-8 border-none shadow-2xl bg-slate-900 text-white rounded-[3rem] relative overflow-hidden group/alert">
                        <div className="absolute inset-0 bg-gradient-to-br from-primary-900 to-slate-900 opacity-50"></div>
                        <div className="relative z-10">
                            <h4 className="flex items-center gap-3 font-black uppercase tracking-[0.2em] text-[10px] text-primary-400 mb-6 italic">
                            ...
                            <button className="w-full py-4 bg-primary-600 hover:bg-primary-500 text-white rounded-2xl font-black uppercase tracking-widest text-[9px] transition-all shadow-xl shadow-primary-900/50 flex items-center justify-center gap-2 group-hover/alert:translate-y-[-2px]">
